...
from IMMORTAL_MUSIC.utils.database import add_sudo, remove_sudo
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer(" ðš´ ðŽ ð ðš° ð“ ðš² á´‹á´€ ð—Ÿá´œÉ´á´… ð—Ÿá´‡É¢á´€ ðŸ˜‚ \n ð—¦á´œá´…á´ÊŸÉªsá´› ð——á´‡á´‹ÊœÉ´á´‡ á´€á´€Êá´€ á´á´„ ðŸ¤§ðŸ–•", show_alert=True)
    else:
        user = await app.get_users(OWNER_ID)

        user_mention = (user.first_name if not user.mention else user.mention)
        caption = f"**Ë¹ÊŸÉªsá´› á´Ò“ Ê™á´á´› á´á´á´…á´‡Ê€á´€á´›á´Ê€sË¼**\n\n**ðŸŒ¹Oá´¡É´á´‡Ê€** âž¥ {user_mention}\n\n"
        ...
        for user_id in SUDOERS:
            if user_id != OWNER_ID:
                try:
                    ...
                    user_mention = user.mention if user else f"**ðŸŽ Sá´œá´…á´ {count} Éªá´…:** {user_id}"
                    caption += f"**ðŸŽ Sá´œá´…á´** {count} **Â»** {user_mention}\n"
