...
from IMMORTAL_MUSIC import app  # Replace with actual import
...
async def interval_selected(client, query):
    ...
    await query.message.reply_text(f"âœ… Quiz started! New quiz every {label}.")
...
async def quiz_stop(client, message):
    ...
    if not quiz_loops.get(user_id):
        await message.reply_text("âŒ No quiz is running.")
        return

    quiz_loops.pop(user_id)
    await message.reply_text("ðŸ›‘ Quiz loop stopped.")
