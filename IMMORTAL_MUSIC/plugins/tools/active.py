from pyrogram import filters
from pyrogram.types import Message
from unidecode import unidecode

from IMMORTAL_MUSIC import app
from IMMORTAL_MUSIC.misc import SUDOERS
from IMMORTAL_MUSIC.utils.database import (
    get_active_chats,
    get_active_video_chats,
    remove_active_chat,
    remove_active_video_chat,
)


@app.on_message(filters.command(["ac"]) & SUDOERS)
async def ac(c, m):
    audio = len(await get_active_chats())
    video = len(await get_active_video_chats())
    await m.reply_text(f"á´€á´œá´…Éªá´ - {audio}\ná´ Éªá´…á´‡á´ - {video}")
    
@app.on_message(
    filters.command(
        ["activevc", "activevoice"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]
    )
    & SUDOERS
)
async def active_voice_chats(client, message):
    ic = await message.reply_text("Â» É¢á´‡á´›á´›ÉªÉ´É¢ á´€á´„á´›Éªá´ á´‡ á´ á´Éªá´„á´‡ á´„Êœá´€á´›s ÊŸÉªsá´›...")
    served_chats = await get_active_chats()
    text = ""
    j = 0
    for x in served_chats:
        try:
            title = (await app.get_chat(x)).title
        except:
            await remove_active_chat(x)
            continue
        try:
            chat = await app.get_chat(x)
            if chat.username:
                user = chat.username
                text += f"<b>{j + 1}.</b> <a href=https://t.me/{user}>{unidecode(title).upper()}</a> [<code>{x}</code>]\n"
            else:
                text += f"<b>{j + 1}.</b> {unidecode(title).upper()} [<code>{x}</code>]\n"
            j += 1
        except:
            continue
    if not text:
        await ic.edit_text(f"Â» É´á´ á´€á´„á´›Éªá´ á´‡ á´ á´Éªá´„á´‡ á´„Êœá´€á´›s á´É´ {app.mention}.")
    else:
        await ic.edit_text(
            f"<b>Â» ÊŸÉªsá´› á´Ò“ á´„á´œÊ€Ê€á´‡É´á´›ÊŸÊ á´€á´„á´›Éªá´ á´‡ á´ á´Éªá´„á´‡ á´„Êœá´€á´›s :</b>\n\n{text}",
            disable_web_page_preview=True,
        )
@app.on_message(
    filters.command(
        ["activev", "activevideo"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]
    )
    & SUDOERS
)
async def activevi_(_, message: Message):
    mystic = await message.reply_text("Â» É¢á´‡á´›á´›ÉªÉ´É¢ á´€á´„á´›Éªá´ á´‡ á´ Éªá´…á´‡á´ á´„Êœá´€á´›s ÊŸÉªsá´›...")
    served_chats = await get_active_video_chats()
    text = ""
    j = 0
    for x in served_chats:
        try:
            title = (await app.get_chat(x)).title
        except:
            await remove_active_video_chat(x)
            continue
        try:
            if (await app.get_chat(x)).username:
                user = (await app.get_chat(x)).username
                text += f"<b>{j + 1}.</b> <a href=https://t.me/{user}>{unidecode(title).upper()}</a> [<code>{x}</code>]\n"
            else:
                text += (
                    f"<b>{j + 1}.</b> {unidecode(title).upper()} [<code>{x}</code>]\n"
                )
            j += 1
        except:
            continue
    if not text:
        await mystic.edit_text(f"Â» É´á´ á´€á´„á´›Éªá´ á´‡ á´ Éªá´…á´‡á´ á´„Êœá´€á´›s á´É´ {app.mention}.")
    else:
        await mystic.edit_text(
            f"<b>Â» ÊŸÉªsá´› á´Ò“ á´„á´œÊ€Ê€á´‡É´á´›ÊŸÊ á´€á´„á´›Éªá´ á´‡ á´ Éªá´…á´‡á´ á´„Êœá´€á´›s :</b>\n\n{text}",
            disable_web_page_preview=True,
        )

