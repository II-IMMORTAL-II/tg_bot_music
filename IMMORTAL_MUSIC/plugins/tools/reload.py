...
from IMMORTAL_MUSIC.utils.formatters import alpha_to_int, get_readable_time
import config
             ...
             [
                 [
                      InlineKeyboardButton(
                         "â€¢ ð–§á´€á´„á´‹á´‡á´… Ð²Ê  â€¢", url=f"https://t.me/{config.OWNER_USERNAME}")
                 ]
            ]
         ),
     )
...
async def close_menu(_, query: CallbackQuery):
    try:
        ...
        umm = await query.message.reply_text(
            f"á´„ÊŸá´êœ±á´‡ Ê™Ê : {query.from_user.mention}"
        )
