...
class YouTubeAPI:
    ...
    async def track(self, link: str, videoid: Union[bool, str] = None):
        ...
        # Primary search path (existing behavior).
        try:
            results = VideosSearch(link, limit=1)
        ...
        # Fallback: yt-dlp metadata search is more reliable for plain text queries.
        ydl_opts = {
            "quiet": True,
            "no_warnings": True,
            "noplaylist": True,
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            if re.search(self.regex, link):
                ...
                search = ydl.extract_info(f"ytsearch1:{link}", download=False)
        ...
        vidid = info.get("id")
        if not vidid:
            raise ValueError("Unable to resolve video id")
        track_details = {
            "title": info.get("title") or "Unknown Title",
            "link": info.get("webpage_url") or f"{self.base}{vidid}",
            "vidid": vidid,
